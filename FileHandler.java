...
public class FileHandler {
    ...
    private static final String COMPLAINTS_FILE = "data/complaints.csv";
    ...
    public static List<User> loadUsers() {
        ...
        readLines(USERS_FILE).forEach(line -> {
            if (!line.isBlank()) list.add(User.fromCsv(line));
        });
        return list;
    }
    ...
    public static List<Complaint> loadComplaints() {
        ...
        readLines(COMPLAINTS_FILE).forEach(line -> {
            if (!line.isBlank()) list.add(Complaint.fromCsv(line));
        });
        return list;
    }
    ...
    public static void saveUsers(Collection<User> users) {
        writeLines(USERS_FILE, users.stream().map(User::toCsv).toList());
    }

    public static void saveComplaints(Collection<Complaint> complaints) {
        writeLines(COMPLAINTS_FILE, complaints.stream().map(Complaint::toCsv).toList());
    }
